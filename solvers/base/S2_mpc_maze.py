    ...
    *,
    ...
    for idx, sc in enumerate(scenarios):
        ...
        '''What is an instance:
            ...
            - B: Controllol input matrix (2x1)
            - rects: List of rectangles (obstacles), each defined by (xmin, ymin, xmax, ymax)
            - start: Starting position (x0, y0)
            ...
            - gamma: CBF constraint parameter
            - goal_tol: Tolerance for considering goal reached
            - collision_margin: Margin for considering collision with rectangles

        What is a solution:
            - states: List of (x,y) states at each time step (including initial state)

        What is the difficulty:
            - Number of rectangles
            - Size of rectangles (smaller gaps are harder)
            - Distance from start to goal
            
            - Formula: difficulty = ((number_of_rectangles) * (average_rectangle_area) / (distance_start_to_goal)) * (1 + (1 / goal_tol)) * (1 + (1 / collision_margin))
            - Formula (alternative): difficulty = (number_of_rectangles) * (average_rectangle_area) / (total_area) * (1 + (1 / goal_tol)) * (1 + (1 / collision_margin))
            - Formula (alternative 2): difficulty = number_of_rectangles_in_line_from_start_to_goal * (1 + (1 / goal_tol)) * (1 + (1 / collision_margin))

        What is correctness:
            - success (boolean) = collision_free AND goal_reached
            - correctness = 1.0 if success else 0.0
            - correctness_for_s1_quality = trajectory_cost (amount of control and distance to the goals -- both the lower the better) scaled also by number/quantinty (how much the agent is inside the obstacles) of collisions (lower is better, can be used to evaluate S1 quality even if not successful)
        
        Notion of confidence:
            - confidence_1 = similarity from S1 -> Against all the stored dynamics (and this is diversity and is avg)
            - confidence_2 = similarity from S1 -> Against all the stored maps (co-sine similiarty avg)
            - confidence_3 = similarity from S1 -> Against the dynamics we used (only one, still diversity)
            - confidence_4 = similarity from S1 -> Against the map we used (only one, still co-sine similiarty)
